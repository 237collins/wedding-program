...
const Navigation = ({ currentPage, totalPages, onPrevious, onNext, theme }: NavigationProps) => {
  ...
  const pageIndicatorVariants = {
    inactive: { 
      scale: 1,
      backgroundColor: theme === 'dark' ? '#f97316' : '#292C43'
    },
    active: { 
      scale: 1.5,
  ...
  return (
    <div className="fixed bottom-4 md:bottom-8 left-1/2 transform -translate-x-1/2 flex flex-col items-center gap-4 z-50 w-full max-w-6xl px-4">
      ...
      {/* <div className="md:hidden flex items-center gap-2 px-4 py-2 rounded-full bg-white/80 dark:bg-[#1a0a0a]/80 backdrop-blur-sm border border-[#E2725B]/20">
        <span className="text-sm font-medium text-[#292C43] dark:text-[#E2725B]">
      ...
      <div className="flex items-center justify-between w-full md:w-auto gap-4 md:gap-8">
        ...
        <motion.button
          ...
          className={`group relative px-6 py-3 rounded-full flex items-center gap-3 transition-all duration-300 overflow-hidden ${
            isFirstPage
              ? theme === 'dark'
                ? 'bg-[#2a0a0a]/50 text-gray-500 cursor-not-allowed border border-[#292C43]/30'
          ...
          whileTap="tap"
        >
          {/* linear background for active button */}
          {!isFirstPage && (
            <motion.div
              className="absolute inset-0 bg-linear-to-r from-[#2435b3] to-[#292C43]"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
        ...
        >
          ...
          {!isLastPage && (
            <motion.div
              className="absolute inset-0 rounded-full border-2 border-[#292C43]/50"
              initial={{ scale: 1, opacity: 0 }}
              whileHover={{
                scale: 1.1,
                opacity: 1,
